"""Render and validate the CC_FIN slice-3 handoff core.

The committed output tree is content commit A's byte authority. It contains the
seven authorized creates plus the single generated BOARD.md modification from
M2 amendment 2. Reproduce it with a reviewed UTC timestamp:

    python render_slice3.py --timestamp YYYY-MM-DDTHH:MM:SSZ
"""
...
OUT = WIKI / "doc/support-transfer/rendered/slice3"
...
parser.add_argument(
    "--timestamp",
    help="fixed UTC BOARD/bootstrap timestamp (YYYY-MM-DDTHH:MM:SSZ)",
)
...
pointer = (
    ...
    "`scripts/make_handoff.py`; publication atomically replaces this pointer.\n"
)

handoff_readme = (
    "# Immutable handoff records\n\n"
    "Tracked bootstrap placeholder for validated immutable handoff records created by "
    "`scripts/make_handoff.py`. No record exists at installation time; `HANDOFF.md` "
...
continuity = continuity.replace("{{PROJECT_NAME}}", "CC_FIN")
...
with tempfile.TemporaryDirectory(prefix="slice3-render-") as tmp:
    target = Path(tmp) / "target"
    shutil.copytree(SLICE2, target)
    # Seed the already-published append-only log only for disposable publisher tests.
    log_path = target / "support/log.md"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    log_path.write_text(
        (SLICE1 / "support/log.md").read_text(encoding="utf-8"),
        encoding="utf-8",
        newline="\n",
    )
    for rel, text in outputs.items():
    ...
    # Exercise the installed publisher from a separate disposable target. The first
    # publication must replace the bootstrap pointer, append the target event shape,
    # make BOARD stale, and validate again only after regeneration.
    probe = Path(tmp) / "probe"
    shutil.copytree(target, probe)
    sys.path.insert(0, str(probe / "scripts"))
    spec = importlib.util.spec_from_file_location(
        "slice3_target_make_handoff", probe / "scripts/make_handoff.py"
    )
    ...
    record = module.HandoffRecord(
        record_id="2026-07-18T131545Z-abc1234",
        created_at_utc="2026-07-18T13:15:45Z",
        source_agent="codex",
        status="partial",
    ...
    stale = subprocess.run(
        [sys.executable, str(probe / "scripts/agent_coord.py"), str(probe)],
        text=True,
        capture_output=True,
        check=False,
    )
    ...
    refreshed = subprocess.run(
        [*board_command[:2], str(probe), *board_command[3:]],
        text=True,
        capture_output=True,
        check=False,
    )
