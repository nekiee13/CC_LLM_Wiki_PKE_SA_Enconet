"""Portable coordination validator and board renderer (T4 design contract).

Validates a target-local ``coordination/`` tree (``messages/``, ``archive/``,
``claims/``, ``schemas/``, ``BOARD.md``) against
``T4_COORDINATION_BOOTSTRAP_CONTRACT.md``. Parameterized entirely by a target
root so the same module can run against a disposable fixture before any
target installation (T4.1-artifact) and later be copied into CC_FIN or
CC_Loto unchanged (T4.2/T7/T8). Stdlib plus PyYAML and jsonschema; no import
of, or path reference to, anything outside this ``staged/`` package.

Every failure mode listed in the T4 "Validator and test contract" section is
represented by one check below; the section heading in this file mirrors that
list so the two stay easy to cross-read.
"""
...
def render_board(root: Path) -> str:
    ...
    lines = ["# BOARD (generated)", "",
             "Generated by coordination_validator.render_board. Never hand-edit; never "
