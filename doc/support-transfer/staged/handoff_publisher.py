"""Portable handoff publisher (T5 design contract).

Implements the ``T5_HANDOFF_CONTINUITY_CONTRACT.md`` deterministic publication
protocol: record-before-pointer atomicity, truthful check-state validation,
Git staleness comparison without fabrication, a sensitive-content boundary,
and named fault-injection checkpoints so disposable-root tests can prove every
interruption outcome the contract requires. Parameterized entirely by a
target root; no import of, or path reference to, anything outside this
``staged/`` package.

Target-local layout produced under ``root``::

    HANDOFF.md                         # replaceable pointer
    support/handoffs/<record_id>.md    # immutable validated records
    support/log.md                     # append-only publication event log

Rendering format: frontmatter carries the four scalar identity fields
(``record_id``, ``created_at_utc``, ``source_agent``, ``status``) exactly as
``handoff-record.template.md`` specifies; each required heading holds either
a Markdown bullet list or a fenced ``json`` block for structured fields
(checks, git/index state, exact next action). The publisher parses both the
frontmatter and the headings back into the same normalized object it wrote,
per the T5 contract -- it does not rely on frontmatter alone.
"""
