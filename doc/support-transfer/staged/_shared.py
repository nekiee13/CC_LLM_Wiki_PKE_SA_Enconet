"""Shared helpers for the T4/T5/T6 staged executable prototypes.

Exactly two consumers: ``coordination_validator.py`` and ``handoff_publisher.py``
(T4/T5 design contracts require two demonstrated consumers before a shared
helper is justified). Stdlib plus PyYAML and jsonschema. No import of, or path
reference to, anything outside this ``staged/`` package: these modules must
run unmodified once copied into a target's ``coordination/``/``support/`` tree
(T3.4 clone-complete rule) -- test fixtures, not this module, are responsible
for seeding schema files from the Wiki templates.
"""
