# MASTER DEVELOPMENT PLAN v1.4 — NQA Supplier Audit Wiki (PKE-SA)

## 0. Document control

| Field | Value |
|---|---|
| Project | 03_PKE_SA_NQA1 / Enconet (formerly Project03 / PKE_SA_Enconet) |
| Plan name | Master_development_plan |
| Version | 1.4 — canonical (published per ADR-0006; supersedes all prefixed variants, now in `docs/_archive/`) |
| Date | 2026-07-11 |
| Status | approved baseline — reconciled and all §8 decisions recorded 2026-07-11 (`decisions/` ADR-0001…0013) |
| Author | Claude (original plan) + Codex (relocation, CC reconciliation, dual-agent additions) |
| Supersedes | docs/_archive/CC_PKE_NQA_Supplier_Audit_Wiki_Enconet_Upgrade_Plan.md (draft input) and the archived plan variants (OG_ v1.1, CC_ v1.2-CC, CX_ v1.3-CX) |
| Amendments | **A1 / v1.1:** shared handoff Task 0.7, Task 0.2/14.3 integration, commons path correction. **A2 / CC v1.2 proposal:** workspace re-root, version consistency, amendment log, preparation inputs. **A3 / CX v1.3 candidate:** Codex `AGENTS.md`/`.agents` added beside Claude Code surfaces; CC findings reconciled through `docs/CX_CC_RECONCILIATION.md`. **A4 / v1.4 (this publication):** decided ADRs folded in — trilingual policy sl/en/hr with per-run deliverable language and no translation layer (ADR-0009: principle 10, Tasks 1.3/1.4/5.5, EPICs 11–12); undeterminable supplier scope = non-conformance (ADR-0012: Tasks 8.1/8.6); §8 open decisions closed to the ADR register. **A5 / 2026-07-11:** Claude Code `/handoff` skill relocated to user-global `~/.claude/skills/handoff/` (ADR-0014; Task 0.7 updated). **A6 / 2026-07-11:** Codex `/handoff` skill relocated to user-global `~/.agents/skills/handoff/` (CX_ADR-0015; Task 0.7 updated). |
...
11. `docs/CX_ALIGNMENT_PLAN.md`, `docs/CX_PREPARATION_CRITIQUE.md`, and
    `docs/CX_CC_RECONCILIATION.md` — relocation/integrity backlog and agreed CC/CX findings.
12. `docs/CC_ALIGNMENT_PLAN.md` and `docs/CC_PREPARATION_CRITIQUE.md` — independent follow-up;
    accepted corrections and additions are recorded in the reconciliation document.
...
7. **Human gates own the state.** Codex and Claude Code may execute work; the human approves phase advances.
...
10. **Trilingual policy, verbatim evidence (ADR-0009).** Only Slovenian, English, and
    Croatian occur. Raw documents are ingested as-is; evidence quotes are always verbatim
    as they exist in the raw documents; there is no translation layer. Deliverables are
    generated in one of the three languages, selected per run by the human
    (`deliverable_language`), and the selection is recorded in run metadata.
...
This reconciled layout separates shared workspace configuration from Enconet-owned artifacts.
Codex and Claude Code use their native discovery surfaces; shared semantics are drift-validated.

```text
03_PKE_SA_NQA1/                    ← workspace level shared by supplier/project entries
├── AGENTS.md                      ← shared Codex guidance
├── CLAUDE.md                      ← shared Claude Code guidance
├── README.md                      ← workspace map and project registry
├── doc/                           ← global architecture, AS-IS, AFI, records, skills, indexing
├── .agents/skills/                ← workspace-shared Codex skills; handoff is user-global
├── .claude/skills/handoff/        ← shared Claude Code handoff skill/counterpart
├── scripts/                       ← workspace validators + deterministic handoff helper
├── tests/                         ← workspace contract and dual-agent drift tests
└── Enconet/                       ← THIS PROJECT
    ├── AGENTS.md                  ← Enconet Codex additions; inherits ../AGENTS.md
    ├── CLAUDE.md                  ← Enconet Claude additions; inherits ../CLAUDE.md
    ├── MASTER_DEVELOPMENT_PLAN.md ← this file after approval
    ├── project-state.yml          ← phase + gate state (EPIC 0 / 14)
    ├── HANDOFF.md                 ← pointer to latest immutable handoff
    ├── handoffs/                  ← immutable session handoff records
    ├── decisions/                 ← immutable ADRs
    ├── docs/                      ← controlled project docs + classified context input
    ...
    │   ├── DATA/                  ← approved fixtures/controlled input per DATA policy
    │   ├── tests/  tools/
    │   └── PROVENANCE.md          ← upstream origin + divergence log
    ├── .agents/skills/            ← Enconet-only Codex skills
    ├── .claude/skills/            ← Enconet-only Claude Code skills/commands
    ├── scripts/                   ← pipeline + validators + generators
    ├── wiki/                      ← index/log/status + controlled knowledge pages
    ├── manifests/  templates/  benchmarks/
...
| 17 | Agent command/workflow interfaces | Claude commands and Codex workflows route to the same validated stages | 14 |
...
### Task 0.2 — Write shared and project agent guidance

**What:** Parallel native discovery files for both supported agents: workspace/project
`AGENTS.md` for Codex and workspace/project `CLAUDE.md` for Claude Code. They carry equivalent
authority, evidence, validation, session, and recordkeeping rules plus documented tool-specific
instructions.
**Why:** Each agent must load durable guidance through its own supported discovery mechanism;
one agent's filename must not be assumed to configure the other.
**Deliverable:** `03_PKE_SA_NQA1/AGENTS.md`, `03_PKE_SA_NQA1/CLAUDE.md`,
`Enconet/AGENTS.md`, and `Enconet/CLAUDE.md`.

- [x] Codex workspace/project `AGENTS.md` files exist; project rules extend workspace rules.
- [x] Claude Code workspace/project `CLAUDE.md` files exist; project rules extend workspace rules.
- [ ] Project rules may specialize but never weaken workspace evidence, safety, record-integrity,
      or validation rules.
...
- [ ] Skills rule: Codex uses `.agents/skills`; Claude Code uses `.claude/skills`. Shared and
      Enconet-specific ownership is explicit (Task 18.7).
- [ ] Session start protocol: native agent guidance → HANDOFF.md → wiki/current-status.md → wiki/index.md →
...
- [ ] A validator detects semantic drift between equivalent dual-agent rules and skills while
      allowing documented tool-specific differences.
...
- [ ] Headers documented in project guidance and the canonical global/project docs.
...
**Deliverable:** `.gitignore`, convention section in both project agent-guidance files.
...
**What:** Provide the same project-neutral handoff contract through the user-global
`~/.agents/skills/handoff/SKILL.md` (Codex; CX_ADR-0015) and the user-global
`~/.claude/skills/handoff/SKILL.md` (Claude Code — relocated from the workspace so all
Claude projects can use it; ADR-0014), backed by one deterministic helper/schema. It
creates an immutable timestamped record and refreshes `HANDOFF.md` atomically.
...
- [x] User-global Codex `SKILL.md` exists and passes the Codex skill validator.
- [x] Claude Code skill counterpart exists and references the same record contract.
...
- [ ] Add `/handoff` to shared/project `AGENTS.md` and `CLAUDE.md`, `/audit-close`, and the
...
- [ ] Both agent integrations pass against Enconet and a fixture second project without workflow forks.
...
languages (`sl, en, hr` — used by both `quote_language` and the per-run
`deliverable_language`; ADR-0009),
...
requirements, and language fields (`quote_original`, `quote_language ∈ {sl, en, hr}`;
no translation fields — ADR-0009).
...
- [ ] Language fields defined against the languages vocabulary (ADR-0009); no translation-status machinery.
...
(move + write-lock via platform-appropriate controls) — procedure written in canonical docs and
referenced from both agent-guidance files, executed manually or via
...
- [ ] Procedure documented (including Windows and POSIX controls) and linked from AGENTS.md/CLAUDE.md.
...
### Task 5.5 — Evidence language fields *(amended A4 per ADR-0009)*

**What:** Importer + schema support for `quote_original` and `quote_language` (sl | en | hr).
No translation layer: `statement_en`, `translation_status`, and `meaning_flag` are removed
from the contract.
**Why:** Evidence occurs in exactly three languages (Slovenian, English, Croatian). The
verbatim quote as it exists in the raw document is the legal fact; deliverable-language
rendering is a generation-time concern (EPICs 11–12), never a stored transformation of
evidence.

- [ ] Original quote always stored verbatim, byte-exact from the source.
- [ ] `quote_language` from the languages vocabulary; missing/invalid value → validation FAIL.
...
- [ ] Not-applicable justification cites the supplier scope source (document or crumb ID)
      **from `raw/`** — scope must be clearly determinable from the raw documents (ADR-0012).
- [ ] If supplier scope cannot be determined from the raw documents: a non-conformance
      finding is raised (scope documentation gap), not-applicable rulings are blocked, and
      the affected criteria are evaluated as applicable against available evidence (ADR-0012).
...
- [ ] Not-applicable ruling without a raw-document scope citation → FAIL (ADR-0012).
...
- [ ] Report rendered in the run's `deliverable_language` (sl | en | hr, ADR-0009); the
      language is recorded in run metadata and stated on the report; quotes stay verbatim.
...
- [ ] Dashboard UI text rendered in the run's `deliverable_language` (sl | en | hr,
      ADR-0009), same language as the paired report; evidence quotes stay verbatim.
...
**What:** Session start: read the active agent's shared/project guidance → latest immutable handoff/HANDOFF.md →
...
- [ ] Session end checklist and `/handoff` are documented for both agents and referenced by `/audit-close`.
...
## EPIC 17 — Agent command and workflow interfaces
...
### Task 17.2 — Command reference in agent guidance

**What:** Claude command table in project `CLAUDE.md` and equivalent direct CLI/workflow routing
in project `AGENTS.md` (name → stage → phase → artifacts).
...
### Task 17.3 — Codex workflow integration

**What:** Codex uses repo-local skills for repeatable, context-rich workflows and direct documented
CLI commands for simple stage invocations. Do not manufacture one skill per slash command when a
tested command is sufficient.

- [ ] `AGENTS.md` maps each pipeline stage to the same scripts, preconditions, and artifacts as
      the Claude command interface.
- [ ] Complex reusable workflows use `.agents/skills/`; simple commands remain direct CLI calls.
- [ ] Cross-agent contract tests prove both interfaces invoke the same validated implementation.
...
- [ ] Both AGENTS.md and CLAUDE.md mark the playbook as mandatory reading before any sieving change.
...
**What:** Three equivalent workflow skills exposed through `.agents/skills/` for Codex and
...
sessions (Project02's mandatory-reading pattern, made native to both agents).

- [ ] Three SKILL.md seeds exist for each agent with the `sieving-` prefix and are cross-linked
      to `sieving/` artifacts (PLAYBOOK, spec guide, CHANGELOG).
- [ ] AGENTS.md and CLAUDE.md rule: the matching skill is read before the corresponding sieving
      activity (run / crumb review / prompt change).
- [ ] A semantic drift validator compares equivalent workflow requirements while allowing
      documented agent-specific invocation details.
...
# 8. Decisions (closed 2026-07-11 — records in `decisions/`)

All six original open decisions are decided and recorded as immutable ADRs; the full
register, including the workspace-level D-decisions, is `decisions/README.md`.

1. **Supplier / framework label** → `enconet` + `appendix_b`; naming scheme
   `<supplier>_<framework>_<artifact>`, extensible to future frameworks such as ISO 9001
   once Appendix B evaluation matures (ADR-0008).
2. **Scoring model calibration** → deliberately deferred to Gate G3; placeholder values
   stay marked `pending human calibration approval` (ADR-0013).
3. **Language policy** → Slovenian/English/Croatian only; no translation layer; quotes
   verbatim; deliverables generated in one user-selected language per run
   (ADR-0009; folded in as amendment A4).
4. **DB engine** → SQLite confirmed (ADR-0010).
5. **Evidence page granularity** → 18 per-criterion pages (ADR-0011).
6. **Supplier scope source** → scope must be clearly determinable from the raw documents;
   an undeterminable scope is a non-conformance finding and blocks not-applicable rulings
   (ADR-0012; folded in as amendment A4).

---

*End of Master Development Plan v1.4 — canonical. v1.0 generated 2026-07-04; amended,
reconciled, and published with all preparation decisions recorded on 2026-07-11
(`docs/CX_CC_RECONCILIATION.md`, `decisions/ADR-0001…0013`). The start gate is open —
EPIC 0 / alignment wave G0 begins.*
