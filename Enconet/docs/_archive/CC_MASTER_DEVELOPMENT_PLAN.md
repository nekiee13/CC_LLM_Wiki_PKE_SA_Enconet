# MASTER DEVELOPMENT PLAN v1.2-CC (proposed) — NQA Supplier Audit Wiki (PKE-SA)
...
| Version | 1.2-CC — proposed revision in `CC_MASTER_DEVELOPMENT_PLAN.md`; replaces `MASTER_DEVELOPMENT_PLAN.md` as v1.2 upon human approval |
| Date | 2026-07-11 |
...
| Amendments | **A1** (2026-07-11 → v1.1): shared `/handoff` skill added as Task 0.7; Tasks 0.2 / 14.3 integrated with it; workspace-commons path corrected to `../CLAUDE.md`. **A2** (2026-07-11 → this revision): §4 re-rooted at `03_PKE_SA_NQA1/Enconet/` with the workspace level made explicit; `handoffs/`, `decisions/`, `HANDOFF.md` added to the project tree; version strings unified across title / control / footer; this amendment log added; 2026-07-11 preparation documents linked as inputs. |
...
11. `docs/CC_ALIGNMENT_PLAN.md` + `docs/CC_PREPARATION_CRITIQUE.md`, and
    `docs/CX_ALIGNMENT_PLAN.md` + `docs/CX_PREPARATION_CRITIQUE.md` — 2026-07-11
    machine-relocation preparation: structure/ownership alignment, `/handoff` contract
    (detail for Task 0.7), record-keeping system, verified-defect register.
...
**Amendment A2.** The tree is rooted in the `03_PKE_SA_NQA1` workspace: the shared
contract, global docs, and the shared `/handoff` skill (Task 0.7) live one level above
the project so Ekonerg / TEKOL / future entries reuse them without copying. Full
ownership model: `docs/CC_ALIGNMENT_PLAN.md` §1.

```text
03_PKE_SA_NQA1/                    ← workspace level (shared across supplier entries)
├── CLAUDE.md                      ← shared workspace contract (Task 0.2 extends it)
├── doc/                           ← global docs: ARCHITECTURE, AS-IS, AFI, RECORD-KEEPING, …
├── .claude/skills/handoff/        ← shared /handoff skill (Task 0.7)
├── scripts/                       ← workspace validators + make_handoff.py helper
└── Enconet/                       ← THIS PROJECT (formerly rooted as PKE_SA_Enconet/)
    ├── CLAUDE.md                  ← project control schema (EPIC 0)
    ├── MASTER_DEVELOPMENT_PLAN.md ← this file
    ├── project-state.yml          ← phase + gate state (EPIC 0 / 14)
    ├── HANDOFF.md                 ← pointer to latest handoff record (Task 0.7)
    ├── handoffs/                  ← immutable session handoff records (Task 0.7)
    ├── decisions/                 ← immutable ADRs — design decisions and trade-offs
    ...
    ├── .claude/                   ← commands (EPIC 17) + Enconet-only sieving skills (Task 18.7)
...
*End of Master Development Plan v1.2-CC (proposed) — v1.0 generated 2026-07-04; amended
A1–A2 on 2026-07-11 (see Document control). Review, decide Section 8, and approve to
begin EPIC 0. On approval this file replaces `MASTER_DEVELOPMENT_PLAN.md` as v1.2 and the
`CX_MASTER_DEVELOPMENT_PLAN.md` duplicate is removed (CC_ALIGNMENT_PLAN Task C1.3).*
