...
def import_file(db: Path, json_path: Path, *, run_id: str, strict: bool = False) -> int:
    ...
    with db_util.connect(db) as conn:
        ...
        for ordinal, item in enumerate(items, 1):
            crumb_id = f"CRUMB-{run['doc_id']}-{item['criterion_id']}-{ordinal:04d}"
            ...
            for qordinal, quote in enumerate(quotes, 1):
                db_util.insert(conn, "crumb_quotes", {"quote_id": f"QUOTE-{run['doc_id']}-{ordinal:04d}-{qordinal:02d}",
...
def main() -> int:
    ...
    try:
        ...
        print(f"import_crumbs: PASS - {count} crumb(s)")
        return 0
